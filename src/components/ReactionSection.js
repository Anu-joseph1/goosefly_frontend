...
const ReactionSection = ({ upvotes, comments: initialCommentCount, shares, postId }) => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        ...
        const response = await authFetch(`http://172.16.11.53:8000/all-comments?post_id=${postId}`);
  ...
  const handleUpvote = async () => {
    try {
      ...
      const response = await authFetch(`http://172.16.11.53:8000/upvote-post`, {
        method: 'POST',
        body: JSON.stringify({
          post_id: postId,
  ...
  const handleAddComment = async (commentText) => {
    try {
      ...
      const response = await authFetch('http://172.16.11.53:8000/write-comments', {
        method: 'POST',
        body: JSON.stringify({
          post_id: postId,
          text: commentText
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      await authFetch(`http://172.16.11.53:8000/delete-comment/${commentId}`, {
        method: 'DELETE'
      });
